import { RES_EMOJI } from "./GameEngine";
...
export class UpgradeTreeView {
    ...
    onBuild;
    onCheat;
    selectedLevel = new Map();
    constructor(engine, container, onBuild, onCheat) {
        this.engine = engine;
        ...
        this.container.addEventListener("click", (e) => {
            ...
            const cheatBtn = target.closest(".tree-cheat-btn");
            if (cheatBtn) {
                this.onCheat(cheatBtn.getAttribute("data-id"));
                return;
            }
            const buildBtn = target.closest(".tree-build-btn");
            if (buildBtn && !buildBtn.disabled) {
                this.onBuild(buildBtn.getAttribute("data-id"));
                return;
            }
            const cell = target.closest(".tree-level-cell.unlocked");
            if (cell) {
    ...
    renderNode(n) {
        ...
        if (n.isLevel && selected !== undefined) {
            ...
            if (lvl && lvl.unlocked)
                detailHtml = this.renderDetail(lvl);
        }
        return `
