...
import { CutsceneManager, CutsceneDef } from "./CutsceneManager";
import cutsceneData from "../data/cutscenes.json";

const engine = new GameEngine();
const emojiMap: Record<string, string> = RES_EMOJI;
...
const treeView = new UpgradeTreeView(
    engine,
    treeCanvas,
    (id) => {
...
schedulerList.addEventListener("click", (e) => {
    const target = (e.target as HTMLElement).closest(".active-task");
    if (target) {
...
function getPrestigeLevel(score: number): number {
...
function drawDiveButtons() {
    ...
    engine.diveTiers.forEach((diveDef, tier) => {
        ...
        if (tier > (engine.parameters.maxDepthTier || 1)) return;
...
function drawUpgrades() {
    ...
    engine.availableUpgrades.forEach(upg => {
        if (engine.unlockedUpgrades.has(upg.id)) return;
        if (upg.requires.some(req => !engine.unlockedUpgrades.has(req))) return;
...
function updateUI() {
    ...
    if (treeOverlayOpen) treeView.render();
}
...
function gameLoop(currentTime: number) {
    ...
    if (tracker) tracker.innerText = `Total Wait Time: ${formatTime(engine.totalWaitTime)}`;
    const upgTracker = document.getElementById("upgrades-tracker");
    if (upgTracker) upgTracker.innerText = `Total Upgrades: ${engine.totalUpgrades}`;
    const diveTracker = document.getElementById("dives-tracker");
    if (diveTracker) diveTracker.innerText = `Dives Count: ${engine.totalDives}`;
...
const cutsceneManager = new CutsceneManager(cutsceneData as unknown as CutsceneDef[]);
